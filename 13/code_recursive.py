...
def recursive_check(pattern, required_smudges = 0, row = 1, offset = 0, smudges = 0):
    if row >= len(pattern):
        return [False]
    try:
...
class Puzzle:
    ...
    def p1(self):
        ...
        for pattern in self.input_list:
            ...
            vertical_fold = recursive_check(pattern=this_pattern.pattern_t,required_smudges=0)
            if vertical_fold[0] == True:
                self.p1_solution += this_pattern.width - vertical_fold[1]
                continue
            horizontal_fold = recursive_check(pattern=this_pattern.pattern,required_smudges=0)
    ...
    def p2(self):
        ...
        for pattern in self.input_list:
            this_pattern = Pattern(pattern)
            vertical_fold = recursive_check(pattern=this_pattern.pattern_t,required_smudges=1)
            if vertical_fold[0] == True and vertical_fold[2] == 1:
                self.p2_solution += this_pattern.width - vertical_fold[1]
                continue
            horizontal_fold = recursive_check(pattern=this_pattern.pattern,required_smudges=1)
