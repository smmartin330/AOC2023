...
def candidate_check(node,grid):
    candidates = []
    look = {}
    path_complete = False

    
    shape = grid.pos(node)[1]
    
    match shape:
        case "|":
            # look vertical
            look = look_vertical(node,grid)
        case "-":
            look = look_horizontal(node,grid)
            # look horizontal
        case "L":
            # look up
            look['u'] = look_up(node,grid)
            # look right
            look['r'] = look_right(node,grid)
        case "J":
            # look up
            look['u'] = look_up(node,grid)
            # look left
            look['l'] = look_left(node,grid)
        case "7":
            # look left
            look['l'] = look_left(node,grid)
            # look down
            look['d'] = look_down(node,grid)
        case "F":
            # look right
            look['r'] = look_right(node,grid)
            # look down
            look['d'] = look_down(node,grid)
        case ".":
            # brother, you screwed up. 
            pass
        case "S":
            #look everywhere
            look = look_around(node,grid)     
    ...
    for k,v in look.items():
        ...
        if v != False:
            ...
            value = v[1]
            is_start = (v[1] == 'S')
            is_visited = v[0] in grid.visited_nodes
            if is_visited == True:
                is_last_visited = ( v[0] == grid.visited_nodes[-1])
                if is_start and not is_last_visited:
                    return candidates,coordinates
                
            elif ( direction == 'd' and 'u' in PIPES[value]):
                candidates.append(coordinates)
            elif ( direction == 'u' and 'd' in PIPES[value]):
                candidates.append(coordinates)
            elif ( direction == 'l' and 'r' in PIPES[value]):
                candidates.append(coordinates)
            elif ( direction == 'r' and 'l' in PIPES[value]):
                candidates.append(coordinates)
    
    return candidates,path_complete

class Puzzle():
    def __init__(self,input_text):
        self.input_text = input_text
        self.input_list = input_text.strip().split('\n')
        self.pipemaze = Grid(raw_grid=self.input_list)
        
        self.pipemaze.scan_grid()
        
        
                
    def p1(self):
        self.p1_solution = 0 # The length of the path divided by 2

        
        self.path_nodes = []
        self.start_node = self.pipemaze.unique_values["S"][0]
        
        # At the start, our only candidate node is the start, and we've not yet
        # visited any nodes.
        self.pipemaze.candidate_nodes = deque([self.start_node])
        self.pipemaze.visited_nodes = []
        self.pipemaze.invalid_nodes = []
        loop_closed = False
        node = self.start_node
        while loop_closed == False:
            new_candidates, loop_closed = candidate_check(node,self.pipemaze)
            if loop_closed != False: # If we've closed the loop
                self.pipemaze.visited_nodes.append(node)
                self.pipemaze.visited_nodes.append(loop_closed)
                pass
                break # stop checking. 
            elif len(new_candidates) == 0: # If we've hit a dead end
                self.pipemaze.invalid_nodes.append(node) #node is invalid
                self.visited_nodes = [] # clear the path because no branching means we're back at the start.
            else: # if we have found successful candidates and they have been added
                valid_candidates = [ candidate for candidate in new_candidates if candidate not in self.pipemaze.visited_nodes and candidate not in self.pipemaze.invalid_nodes ]
                self.pipemaze.candidate_nodes.extend(valid_candidates)
                self.pipemaze.visited_nodes.append(node) # We add it to the visited nodes
                node = self.pipemaze.candidate_nodes.pop()

        self.pipemaze.loop = self.pipemaze.visited_nodes
        self.p1_solution = len(self.pipemaze.visited_nodes) // 2
        
        return True

    def p2(self):
        self.pipemaze.draw_loop = ''
        self.pipemaze.loopmap = []
        for y in range(0, self.pipemaze.height):
            this_row = ''
            this_map_row =[]
            for x in range(0, self.pipemaze.width):
                if (x,y) in self.pipemaze.loop:
                    this_map_row.append(self.pipemaze.pos((x,y))[1])
                    this_row+=self.pipemaze.pos((x,y))[1]
                else:
                    this_map_row.append(' ')
                    this_row+=' '
            self.pipemaze.loopmap.append(this_map_row)
            self.pipemaze.draw_loop+=this_row+"\n"
        
        self.pipemaze.loopmap_t = numpy.transpose(self.pipemaze.loopmap).tolist()
        print(self.pipemaze.draw_loop)   
        
        # all_nodes = self.pipemaze.visited_nodes.copy()
        # all_nodes.sort()
        # top_left = all_nodes[0]
        # index = self.pipemaze.visited_nodes.index(top_left)
        # self.pipemaze.new_path = self.pipemaze.visited_nodes[index:] + self.pipemaze.visited_nodes[:index]
        ...
        x_possible = []
        for y in range(0,self.pipemaze.height):
            loopmap_row = self.pipemaze.loopmap[y]
            is_inside = False
            for x in range(0,self.pipemaze.width):
                if loopmap_row[x] in ['|','L','J']:
                    if is_inside == True:
                        is_inside = False
                    else:
                        is_inside = True
                if loopmap_row[x] == ' ':
                    if is_inside == True:
                        x_possible.append((x,y))

            # discard anything on the boundaries
            for x in range(0,self.pipemaze.width):
                if loopmap_row[x] == ' ':
                    try:
                        x_possible.remove((x,y))
                    except:
                        pass
                else:
                    break
            
            for x in range(self.pipemaze.width-1,0,-1):
                if loopmap_row[x] == ' ':
                    try:
                        x_possible.remove((x,y))
                    except:
        ...
        y_possible = []
        # for x in range(0,self.pipemaze.width):
        #     loopmap_t_row = self.pipemaze.loopmap_t[x]
        #     is_inside = False
        #     for y in range(0,self.pipemaze.height):
        #         if loopmap_t_row[y] in ['F', '-', 'L']:
        #             is_inside = bool(~is_inside)
        #         if loopmap_t_row[y] == ' ' and is_inside == True:
        #             y_possible.append((x,y))

        #     # discard anything on the boundaries
        #     for y in range(0,self.pipemaze.height):
        #         if loopmap_t_row[y] == ' ':
        #             try:
        #                 y_possible.remove((x,y))
        #                 self.pipemaze.loopmap[y][x] = " "
        #             except:
        #                 pass
        #         else:
        #             break
            
        #     for y in range(self.pipemaze.height-1,0,-1):
        #         if loopmap_t_row[y] == ' ':
        #             try:
        #                 y_possible.remove((x,y))
        #                 self.pipemaze.loopmap[y][x] = " "
        ...
        # possible = [ dot for dot in x_possible if dot in y_possible ]
        possible = x_possible
        bad_dots = []
        for dot in possible:
            self.pipemaze.loopmap[dot[1]][dot[0]] = 'X'
        for dot in possible:
            x,y = dot[0], dot[1]
            if ((self.pipemaze.loopmap[y-1][x-1] == ' ') or
                (self.pipemaze.loopmap[y-1][x] == ' ') or
                (self.pipemaze.loopmap[y-1][x+1] == ' ') or 
                (self.pipemaze.loopmap[y][x-1] == ' ') or 
                (self.pipemaze.loopmap[y][x+1] == ' ') or
                (self.pipemaze.loopmap[y+1][x-1] == ' ') or 
                (self.pipemaze.loopmap[y+1][x] == ' ') or 
                (self.pipemaze.loopmap[y+1][x+1] == ' ')):
                bad_dots.append(dot)
                self.pipemaze.loopmap[y][x] = ' '
        
        possible = [dot for dot in possible if dot not in bad_dots]
        bad_dots = []
        ...
        for row in self.pipemaze.loopmap:
            for char in row:
                print(char,end='')
            print()
